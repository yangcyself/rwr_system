...
import os
import cvxopt
import faive_system.src.hand_control.hand_control.simplified_finger_kinematics as fk
import sympy as sym
from threading import Thread, RLock, Event
from faive_system.src.hand_control.hand_control.joint_ekf import EKF
from faive_system.src.common.utils import get_datetime_str
from faive_system.src.hand_control.hand_control.dynamixel_client import *
...
class MuscleGroup:
    ...
    """
    attributes = ["joint_ids", "motor_ids", "spool_rad", "wind_direction", "joint_ranges", "motor_init_pos", "calibration_max", "alpha", "joint_radius"]
    def __init__(self, name, muscle_group_json: dict):
        ...
        for idx in range(len(joint_ranges)):
            ...
            assert low_lim < up_lim, f"joint range [idx] {low_lim} < {up_lim} is invalid"
        ...
        print(f"Created muscle group {name} with joint ids {self.joint_ids}, motor ids {self.motor_ids} and spool_rad {self.spool_rad}")
...
class HandController:
    ...
    def __init__(self,
        ...
        if dummy_mode:
            self._dxc = DummyDynamixelClient(self.motor_ids, port, baudrate, lazy_connect=True)
        else:
            self._dxc = DynamixelIndirectClient(start_update_thread=True, motor_ids=self.motor_ids, port=port, baudrate=baudrate, lazy_connect=True)
    ...
    def _load_musclegroup_yaml(self, filename):
        ...
        attrs_to_get = ["joint_ids", "motor_ids", "spool_rad", "wind_direction", "motor_init_pos"]
    ...
    def tendon_pos2motor_pos_sym(self, tendon_lengths, muscle_group):
        ...
        for m_i, motor_id in enumerate(muscle_group.motor_ids):
            motor_pos[m_i] = tendon_lengths[m_i]/(np.abs(muscle_group.spool_rad[m_i]) * muscle_group.wind_direction[m_i])
        return motor_pos
    ...
    def write_desired_motor_current(self, motor_currents_mA):
        ...
        with self.motor_lock:
            self._dxc.write_desired_current(self.motor_ids, - motor_currents_mA * self.directions)
    ...
    def pose2motors_sym(self, joint1, joint2, joint3, joint4=None, muscle_group=None):
        ...
        Output: motor positions 
        """
        tendon_lengths = fk.get_tendon_lengths_lambda(joint1, joint2, joint3, muscle_group)
        motor_pos = self.tendon_pos2motor_pos_sym(tendon_lengths, muscle_group=muscle_group)
        return motor_pos
    ...
    def command_joint_angles(self, joint_angles_deg: np.array):
        ...
        if max_diff > 0.1:  # 0.1 deg
            ...
            motor_pos_des = self.pose2motors(np.deg2rad(joint_angles_deg)) - self.motor_pos_norm + self.motor_init_pos
        ...
        for ekf in self.ekfs:
            ...
            self.joint_pos_array[joint_pos_begin_idx:joint_pos_end_idx], \
                self.joint_vel_array[joint_pos_begin_idx:joint_pos_end_idx] = \
                    ekf.update(motor_pos_array[motor_pos_begin_idx:motor_pos_end_idx], motor_vel_array[motor_pos_begin_idx:motor_pos_end_idx])
            motor_pos_begin_idx = motor_pos_end_idx
            joint_pos_begin_idx = joint_pos_end_idx
